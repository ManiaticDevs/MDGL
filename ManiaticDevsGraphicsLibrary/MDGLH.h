...
public:
	...
	MDSprite(int w, int h)
	{
		Create(w, h);
	}

	MDSprite(wstring sFile)
	{
		if (!Load(sFile))
			Create(8, 8);
	}
...
class MDCGE
{
	...
	static
		LRESULT CALLBACK MDWindProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		static MDCGE* cge;
		...
		{
		case WM_CREATE:
			cge = static_cast<MDCGE*>(((LPCREATESTRUCT)lParam)->lpCreateParams);
	...
	{
		wchar_t wnd_title[] = L"MANIATICDEVS CONSOLE APP";
		wchar_t wnd_class[] = L"MD_CONSOLE_GAME_ENGINE_CLASS";
...
public:
	...
	void DrawSprite(int x, int y, MDSprite* sprite)
	{
		if (sprite == nullptr)
			return;
	...
	void DrawPartialSprite(int x, int y, MDSprite* sprite, int ox, int oy, int w, int h)
	{
		if (sprite == nullptr)
			return;
...
public:
	void Start()
	{
		...
		// Star the thread
		thread t = thread(&MDCGE::GameThread, this);
	...
	void GameThread()
	{
		...
		while (m_bAtomActive)
		{
			...
			while (m_bAtomActive)
			{
				...
				swprintf_s(sNewTitle, 256, L"ManiaticDevs - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
...
protected: // Audio Engine =====================================================================
	...
	public:
		...
		MDAudioSample(std::wstring sWavFile)
		{
	...
	std::vector<MDAudioSample> vecAudioSamples;
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&MDCGE::AudioThread, this);
	...
	{
		((MDCGE*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
atomic<bool> MDCGE::m_bAtomActive(false);
condition_variable MDCGE::m_cvGameFinished;
mutex MDCGE::m_muxGame;